...
// threshold).
[[nodiscard]] inline f32 combined_restitution(const f32 a,
                                              const f32 b) noexcept {
  return std::sqrt(a * b);
}
[[nodiscard]] inline f32 combined_friction(const f32 a, const f32 b) noexcept {
  return std::min(a, b);
}
} // namespace javelin::detail
...
struct PhysicsSystem final {
  ...
  void set_gravity(const f32 gravity) noexcept {
    gravity_.store(gravity, std::memory_order_relaxed);
  }
  ...
  void set_simulation_paused(const bool paused) noexcept {
    const bool previous =
        simulation_paused_.exchange(paused, std::memory_order_acq_rel);
    if (!paused) {
  ...
  void request_simulation_steps(const u32 count = 1u) noexcept {
    ...
    for (;;) {
      ...
      if (pending_step_budget_.compare_exchange_weak(
              pending, next, std::memory_order_acq_rel,
  ...
  [[nodiscard]] f32 gravity() const noexcept {
    return gravity_.load(std::memory_order_relaxed);
  }
  [[nodiscard]] f32 linear_damping() const noexcept {
    return linear_damping_.load(std::memory_order_relaxed);
  }
  [[nodiscard]] f32 angular_damping() const noexcept {
    return angular_damping_.load(std::memory_order_relaxed);
  }
  ...
  [[nodiscard]] AabbDebugSnapshot aabb_debug_snapshot() const noexcept {
    return aabb_debug_channel_.snapshot();
  }
  [[nodiscard]] f32 last_tick_dt_ms() const noexcept {
    return last_tick_dt_ms_.load(std::memory_order_relaxed);
  }
  ...
  [[nodiscard]] static constexpr f32 fixed_step_dt_seconds() noexcept {
    return kFixedStepDtSeconds;
  }

  [[nodiscard]] bool step_fixed() noexcept {
    if (thread_.joinable()) {
      log::warn(physics,
                "step_fixed ignored while threaded simulation is running");
      return false;
    }

    last_tick_dt_ms_.store(static_cast<f32>(kFixedStepDtMilliseconds),
                           std::memory_order_relaxed);
  ...
  void start() {
    ...
    log::info(physics, "Params gravity={} linear_damping={} angular_damping={}",
              gravity(), linear_damping(), angular_damping());
    thread_ = std::jthread([this](const std::stop_token &stop_token) {
      ...
      constexpr auto fixed_step_interval =
          std::chrono::duration_cast<SteadyClock::duration>(
              std::chrono::duration<f64>(kFixedStepDtSeconds));
      ...
      while (!stop_token.stop_requested()) {
        ...
        last_tick_dt_ms_.store(static_cast<f32>(tick_dt_ms),
                               std::memory_order_relaxed);
        ...
        const bool paused_for_plot =
            simulation_paused_.load(std::memory_order_relaxed);
        const u32 pending_steps_for_plot =
            pending_step_budget_.load(std::memory_order_relaxed);
        const u64 completed_steps_for_plot =
            completed_sim_step_count_.load(std::memory_order_relaxed);
        TracyPlot("physics_paused",
                  paused_for_plot ? static_cast<i64>(1) : static_cast<i64>(0));
        TracyPlot("physics_pending_steps",
                  static_cast<i64>(pending_steps_for_plot));
        TracyPlot("physics_completed_steps",
                  tracy_counter_i64_(completed_steps_for_plot));
...
private:
  ...
  static constexpr f32 kClampLinearSpeedThresholdSq =
      kClampLinearSpeedThreshold * kClampLinearSpeedThreshold;
  static constexpr f32 kClampAngularSpeedThresholdSq =
      kClampAngularSpeedThreshold * kClampAngularSpeedThreshold;
  ...
  static constexpr f32 kPersistenceAnchorThresholdSq =
      kPersistenceAnchorThreshold * kPersistenceAnchorThreshold;
  ...
  static constexpr f32 kPersistenceTangentialDriftBreakThresholdSq =
      kPersistenceTangentialDriftBreakThreshold *
      kPersistenceTangentialDriftBreakThreshold;
  ...
  [[nodiscard]] static i64 tracy_counter_i64_(const u64 value) noexcept {
    return static_cast<i64>(std::min<u64>(
        value, static_cast<u64>(std::numeric_limits<i64>::max())));
  }

  [[nodiscard]] static usize
  grown_capacity_(const usize current_capacity,
                  const usize required_capacity) noexcept {
  ...
  static void update_reserve_hint_(const usize observed_size,
                                   usize &reserve_hint) noexcept {
  ...
  [[nodiscard]] static u32 ceil_div_u32_(const u32 numerator,
                                         const u32 denominator) noexcept {
    return (numerator + denominator - 1u) / denominator;
  }

  [[nodiscard]] static BroadPhaseDispatch
  choose_broad_phase_dispatch_(const u32 query_dynamic_count,
                               const u32 max_worker_count) noexcept {
    if (query_dynamic_count == 0u || max_worker_count == 0u) {
      return BroadPhaseDispatch{};
    }

    if (max_worker_count == 1u ||
        query_dynamic_count < kBroadPhaseParallelMinQueries) {
      return BroadPhaseDispatch{.worker_count = 1u,
                                .chunk_size = query_dynamic_count};
    }

    const u32 worker_cap_by_chunk =
        std::max<u32>(1u, query_dynamic_count / kBroadPhaseMinQueriesPerWorker);
    const u32 worker_cap = std::min(max_worker_count, worker_cap_by_chunk);
    const u32 worker_target =
        std::max<u32>(1u, ceil_div_u32_(query_dynamic_count,
                                        kBroadPhaseTargetQueriesPerWorker));
  ...
  void
  reserve_broad_phase_worker_pair_buffers_(const usize pair_capacity_hint) {
  ...
  [[nodiscard]] HotPathCapacitySnapshot
  capture_hot_path_capacity_snapshot_() const noexcept {
    ...
    snapshot.manifold_material_capacity =
        manifold_restitution_cache_.capacity();
  ...
  void publish_hot_path_capacity_growth_(
      const HotPathCapacitySnapshot &before) const noexcept {
    const HotPathCapacitySnapshot after = capture_hot_path_capacity_snapshot_();
    i64 growth_events = 0;
    usize growth_bytes = 0;

    const auto accumulate_growth = [&](const usize before_capacity,
                                       const usize after_capacity,
    ...
    accumulate_growth(before.candidate_pairs_capacity,
                      after.candidate_pairs_capacity, sizeof(BodyPair));
    accumulate_growth(before.manifold_capacity, after.manifold_capacity,
                      sizeof(ContactManifold));
    accumulate_growth(before.manifold_material_capacity,
                      after.manifold_material_capacity, sizeof(f32) * 2u);
    accumulate_growth(before.broad_phase_worker_pair_capacity_sum,
                      after.broad_phase_worker_pair_capacity_sum,
                      sizeof(BodyPair));

    TracyPlot("physics_hot_capacity_growth_events", growth_events);
    TracyPlot("physics_hot_capacity_growth_bytes",
              static_cast<i64>(growth_bytes));
    TracyPlot("physics_candidate_pairs_capacity",
              static_cast<i64>(after.candidate_pairs_capacity));
    TracyPlot("physics_manifold_capacity",
              static_cast<i64>(after.manifold_capacity));
  ...
  [[nodiscard]] bool
  wait_for_simulation_control_event_(const std::stop_token &stop_token) {
    std::unique_lock lock(simulation_control_mutex_);
    simulation_control_cv_.wait(lock, [&] {
      return stop_token.stop_requested() ||
             !simulation_paused_.load(std::memory_order_acquire) ||
  ...
  [[nodiscard]] bool apply_pending_reset_() noexcept {
    ...
    if (scene_ != nullptr) {
      ...
      contact_debug_channel_.publish_empty(
          completed_sim_step_count_.load(std::memory_order_relaxed));
      aabb_debug_channel_.publish_empty(
          completed_sim_step_count_.load(std::memory_order_relaxed));
  ...
  [[nodiscard]] bool simulate_one_fixed_tick_() {
    ...
    const f32 angular_damping =
        angular_damping_.load(std::memory_order_relaxed);
    ...
    const HotPathCapacitySnapshot hot_capacity_before =
        capture_hot_path_capacity_snapshot_();
    prepare_previous_manifolds_();

    // Stage 1: external forces and per-body bounds for broad phase.
    integrate_gravity_velocity(view.velocity, view.inv_mass, view.asleep,
                               gravity, dt);
    f32 max_angular_speed_sq = 0.0f;
    {
      ZoneScopedN("Physics max angular speed scan");
      for (u32 i = 0; i < count; ++i) {
        max_angular_speed_sq = std::max(max_angular_speed_sq,
                                        view.angular_velocity[i].length_sq());
      }
    }
    ...
    bounds_cache_.resize(count);
    {
      ...
      for (u32 i = 0; i < count; ++i) {
        ...
        case ShapeKind::box: {
          ...
          const Vec3 extents = abs0 * box.half_extents.x +
                               abs1 * box.half_extents.y +
    ...
    const std::span<const u32> dynamic_ids{dynamic_ids_.data(),
                                           dynamic_ids_.size()};
    build_awake_dynamic_body_set_(count, view.asleep);
    const std::span<const u32> awake_dynamic_ids{awake_dynamic_ids_.data(),
                                                 awake_dynamic_ids_.size()};
    TracyPlot("physics_dynamic_bodies", static_cast<i64>(dynamic_ids.size()));
    TracyPlot("physics_awake_dynamic_bodies",
              static_cast<i64>(awake_dynamic_ids.size()));
    ...
    const std::span<const u32> bvh_update_ids =
        rebuilt_body_sets ? dynamic_ids : awake_dynamic_ids;
    const std::span<const u8> awake_dynamic_mask{awake_dynamic_mask_.data(),
                                                 count};
    update_dynamic_bvh_and_collect_moved_awake_(bvh_update_ids, count,
                                                awake_dynamic_mask);
    const std::span<const u32> moved_awake_dynamic_ids{
        moved_awake_dynamic_ids_.data(), moved_awake_dynamic_ids_.size()};
    ...
    const bool fallback_to_full_awake_query =
        rebuilt_body_sets ||
        moved_awake_ratio >= kBroadPhaseIncrementalFallbackMovedRatio;
    const std::span<const u32> broad_phase_query_ids =
        fallback_to_full_awake_query ? awake_dynamic_ids
                                     : moved_awake_dynamic_ids;
...
#if defined(JAVELIN_BROAD_PHASE_VALIDATE)
    validate_broad_phase_pairs_(awake_dynamic_ids, awake_dynamic_mask,
                                fallback_to_full_awake_query,
                                next_completed_step_id_());
#endif
    TracyPlot("physics_moved_awake_dynamic_bodies",
              static_cast<i64>(moved_awake_dynamic_ids.size()));
    TracyPlot("physics_moved_awake_ratio", moved_awake_ratio);
    TracyPlot("physics_carried_pair_count",
              static_cast<i64>(carried_pair_count));
    TracyPlot("physics_validated_pair_count",
              static_cast<i64>(validated_pair_count));
    TracyPlot("physics_broad_phase_full_query", fallback_to_full_awake_query
                                                    ? static_cast<i64>(1)
                                                    : static_cast<i64>(0));
    ...
    narrow_phase_contacts(view.position, view.orientation, view.shape_kind,
                          view.shapes, view.shape_index, view.inv_mass,
                          candidate_pairs_, manifolds_, awake_dynamic_ids,
                          next_manifolds_);
    ...
    u32 contact_point_count = 0u;
    {
      ...
      const f32 avg_points_per_manifold =
          (manifold_count > 0u) ? (static_cast<f32>(contact_point_count) /
                                   static_cast<f32>(manifold_count))
                                : 0.0f;
      ...
      TracyPlot("physics_contact_points",
                static_cast<i64>(contact_point_count));
      ...
      TracyPlot("physics_dropped_points",
                static_cast<i64>(persistence_stats.dropped_point_count));
      TracyPlot("physics_axis_flip_count",
                static_cast<i64>(persistence_stats.axis_flip_count));
    ...
    manifold_friction_cache_.resize(manifold_count);
    {
      ...
      for (u32 i = 0; i < manifold_count; ++i) {
        ...
        manifold_restitution_cache_[i] = detail::combined_restitution(
            view.material_restitution[mat_a], view.material_restitution[mat_b]);
        manifold_friction_cache_[i] = detail::combined_friction(
            view.material_friction[mat_a], view.material_friction[mat_b]);
      }
    }
    ...
    // resting contact.
    build_activity_masks_(count, std::span<const ContactManifold>{manifolds_},
                          view.constraints, view.inv_mass);
    const u32 max_dynamic_island_size = build_dynamic_islands_(
        count, view.inv_mass, std::span<const ContactManifold>{manifolds_},
        view.constraints);
    static_cast<void>(wake_sleeping_islands_with_active_edges_(
        std::span<const ContactManifold>{manifolds_}, view.constraints,
    ...
    const ContactSolveConfig contact_solve_config{
        ...
        .max_iterations = complex_contact_solve
                              ? kContactSolverComplexMaxIterations
    ...
    TracyPlot("physics_contact_solver_adaptive_mode",
              complex_contact_solve ? static_cast<i64>(1)
                                    : static_cast<i64>(0));
    TracyPlot("physics_contact_solver_max_iterations",
              static_cast<i64>(contact_solve_config.max_iterations));
    solve_contact_velocities(
        view.velocity, view.angular_velocity, view.inv_mass, view.inv_inertia,
        view.orientation, manifolds_, dt,
        std::span<const f32>{manifold_restitution_cache_},
        std::span<const f32>{manifold_friction_cache_},
        std::span<const u8>{view.asleep}, contact_solve_config);
    solve_contact_penetration(view.position, view.orientation, view.inv_mass,
                              view.inv_inertia,
    ...
    solve_distance_constraints(
        ...
        std::span<const u8>{view.asleep});
    apply_linear_damping(view.velocity, view.inv_mass, view.asleep,
                         linear_damping, dt);
    apply_angular_damping(view.angular_velocity, view.inv_mass, view.asleep,
                          angular_damping, dt);
    ...
    clamp_resting_contact_velocities(
    ...
    update_sleep_timers_(
        count, std::span<const u8>{contact_activity_mask_.data(), count},
    ...
    integrate_positions(view.position, view.velocity, view.inv_mass,
                        view.asleep, dt);
    integrate_orientations(view.orientation, view.angular_velocity,
                           view.inv_mass, view.asleep, dt);
    const bool publish_contact_debug =
        contact_debug_enabled_.load(std::memory_order_acquire);
    if (publish_contact_debug) {
      publish_contact_debug_snapshot_(
          view.position, view.orientation,
          std::span<const ContactManifold>{manifolds_},
          next_completed_step_id_());
    ...
    const bool publish_aabb_debug =
        aabb_debug_enabled_.load(std::memory_order_acquire);
    ...
    publish_poses(view.poses, view.position, view.orientation, view.asleep,
                  count);
  ...
  void ensure_capacity_(const u32 count) {
...
#if defined(JAVELIN_BROAD_PHASE_VALIDATE)
    broad_phase_expected_pairs_.reserve(static_cast<usize>(count) *
                                        kPairReserveFactor);
#endif
    const usize manifold_reserve =
        static_cast<usize>(count) * kManifoldReserveFactor;
    ...
    update_reserve_hint_(candidate_pairs_.capacity(),
                         candidate_pair_reserve_hint_);
  ...
  void prepare_previous_manifolds_() {
    ...
    for (u32 i = 1; i < manifolds_.size(); ++i) {
      ...
      if (!manifold_pair_less_(prev, curr) &&
          !manifold_pair_less_(curr, prev)) {
        log::error(physics,
                   "Duplicate previous manifold pair (index={} pair=({}, {}))",
                   i, curr.a, curr.b);
        std::terminate();
      }
    }
  ...
  void build_awake_dynamic_body_set_(const u32 body_count,
    ...
    for (const u32 id : dynamic_ids_) {
      ...
      if (id >= body_count) {
        log::error(
            physics,
            "Dynamic id out of range while building awake set (id={} count={})",
            id, body_count);
        std::terminate();
      }
#endif
  ...
  void update_dynamic_bvh_and_collect_moved_awake_(
      std::span<const u32> bvh_update_ids, const u32 body_count,
    ...
    for (const u32 id : bvh_update_ids) {
      ...
      if (id >= body_count) {
        log::error(physics,
                   "BVH update id out of range while collecting moved set "
                   "(id={} count={})",
                   id, body_count);
        std::terminate();
      }
  ...
  append_overlapping_previous_pairs_(const u32 body_count,
                                     std::span<const u8> awake_dynamic_mask) {
    ...
    for (const ContactManifold &manifold : manifolds_) {
      ...
      if (pair.a >= body_count || pair.b >= body_count) {
        log::error(physics,
                   "Previous manifold pair out of range during carry-forward "
                   "(a={} b={} count={})",
                   pair.a, pair.b, body_count);
        std::terminate();
      }
#endif
      if (awake_dynamic_mask[pair.a] == 0u &&
          awake_dynamic_mask[pair.b] == 0u) {
        continue;
      }
  ...
  void build_activity_masks_(const u32 body_count,
    ...
    std::fill_n(contact_activity_mask_.begin(), body_count,
                static_cast<u8>(0u));
    std::fill_n(constraint_activity_mask_.begin(), body_count,
                static_cast<u8>(0u));
...
#ifndef NDEBUG
      if (manifold.a >= body_count ||
          (manifold.b != kInvalidBody && manifold.b >= body_count)) {
        log::error(physics,
                   "Contact mask manifold id out of range (a={} b={} count={})",
    ...
    for (const DistanceConstraint &constraint : constraints) {
      ...
      if (constraint.body_a >= body_count || constraint.body_b >= body_count) {
        log::error(physics,
                   "Constraint mask id out of range (a={} b={} count={})",
                   constraint.body_a, constraint.body_b, body_count);
        std::terminate();
      }
#endif
  ...
  [[nodiscard]] u32
  build_dynamic_islands_(const u32 body_count, std::span<const f32> inv_mass,
                         std::span<const ContactManifold> manifolds,
                         std::span<const DistanceConstraint> constraints) {
    ...
    for (const u32 body : dynamic_ids_) {
      ...
      if (body >= body_count) {
        log::error(
            physics,
            "Dynamic id out of range while building islands (id={} count={})",
            body, body_count);
        std::terminate();
      }
#endif
    ...
    for (const ContactManifold &manifold : manifolds) {
      ...
      if (a >= body_count || b >= body_count) {
        log::error(
            physics,
            "Island contact manifold id out of range (a={} b={} count={})", a,
            b, body_count);
        std::terminate();
      }
#endif
    ...
    for (const DistanceConstraint &constraint : constraints) {
      ...
      if (a >= body_count || b >= body_count) {
        log::error(physics,
                   "Island constraint id out of range (a={} b={} count={})", a,
                   b, body_count);
        std::terminate();
      }
#endif
  ...
  void register_sleep_island_for_component_(const u32 component_head) {
    ...
    while (body != kInvalidBody) {
      ...
      if (sleep_island_of_body_[body] != kInvalidIsland) {
        log::error(physics,
                   "Body already belongs to a sleep island (body={} island={})",
                   body, sleep_island_of_body_[body]);
        std::terminate();
      }
#endif
  ...
  [[nodiscard]] u32 wake_sleep_island_(const u32 island_id,
                                       std::span<u8> asleep,
  ...
  [[nodiscard]] IslandWakeStats wake_sleeping_islands_with_active_edges_(
    ...
    TracyPlot("physics_islands_woken",
              static_cast<i64>(stats.woken_island_count));
  ...
  [[nodiscard]] IslandSleepStats
  sleep_awake_islands_(std::span<u32> sleep_timer, std::span<u8> asleep) {
    ...
    TracyPlot("physics_islands_slept",
              static_cast<i64>(stats.slept_island_count));
  ...
  void update_sleep_timers_(const u32 count, std::span<const u8> in_contact,
                            std::span<const Vec3> velocity,
                            std::span<const Vec3> angular_velocity,
                            std::span<const f32> inv_mass,
                            std::span<u32> sleep_timer,
                            std::span<const u8> asleep) noexcept {
    ...
    for (u32 i = 0; i < count; ++i) {
      ...
      const bool at_rest =
          velocity[i].length_sq() <= kSleepLinearSpeedThresholdSq &&
      ...
      if (in_contact[i] != 0u && at_rest) {
        ...
        sleep_timer[i] = (sleep_timer[i] > kSleepTimerDecayPerMiss)
                             ? (sleep_timer[i] - kSleepTimerDecayPerMiss)
                             : 0u;
      }
    }
  }
  ...
  void publish_contact_debug_snapshot_(
    ...
    for (const ContactManifold &manifold : manifolds) {
      ...
      for (u32 point_index = 0u; point_index < manifold.point_count;
           ++point_index) {
        const ContactPoint &point = manifold.points[point_index];
        const Vec3 world_a =
            position[a] + rotate(orientation[a], point.local_anchor_a);

        Vec3 point_world = world_a;
        if (has_body_b) {
          const Vec3 world_b =
              position[b] + rotate(orientation[b], point.local_anchor_b);
        ...
        out.persisted[out_index] =
            point.persisted ? static_cast<u8>(1u) : static_cast<u8>(0u);
        ++out_index;
      }
    }
    ...
    if (out_index != point_count) {
      log::error(physics,
                 "Contact debug packing mismatch expected={} actual={}",
                 point_count, out_index);
      std::terminate();
    }
#endif
  ...
  [[nodiscard]] static bool body_pair_less_(const BodyPair lhs,
                                            const BodyPair rhs) noexcept {
    if (lhs.a != rhs.a) {
      return lhs.a < rhs.a;
    }
    return lhs.b < rhs.b;
  ...
  [[nodiscard]] static bool body_pair_equal_(const BodyPair lhs,
                                             const BodyPair rhs) noexcept {
  ...
  static void normalize_and_sort_body_pairs_(std::vector<BodyPair> &pairs) {
    ...
    const auto unique_end =
        std::unique(pairs.begin(), pairs.end(), body_pair_equal_);
    pairs.erase(unique_end, pairs.end());
  }

  [[nodiscard]] static bool
  manifold_pair_less_(const ContactManifold &lhs,
                      const ContactManifold &rhs) noexcept {
    if (lhs.a != rhs.a) {
      return lhs.a < rhs.a;
    }
    return lhs.b < rhs.b;
  ...
  [[nodiscard]] static bool
  manifold_pair_equal_(const ContactManifold &lhs,
                       const ContactManifold &rhs) noexcept {
    return lhs.a == rhs.a && lhs.b == rhs.b;
  }
  ...
  [[nodiscard]] static bool
  manifold_less_(const ContactManifold &lhs,
                 const ContactManifold &rhs) noexcept {
    if (lhs.a != rhs.a) {
      return lhs.a < rhs.a;
    }
  ...
  contact_point_count_(std::span<const ContactManifold> manifolds) noexcept {
    u32 count = 0u;
    for (const ContactManifold &manifold : manifolds) {
  ...
  [[nodiscard]] static bool
  decode_box_axis_feature_id_(const u32 feature_id, BoxAxisKey &out) noexcept {
  ...
  [[nodiscard]] static bool
  manifold_axis_flipped_(const ContactManifold &previous_manifold,
                         const ContactManifold &next_manifold) noexcept {
    if (previous_manifold.point_count == 0u ||
        next_manifold.point_count == 0u) {
      return false;
    }
    BoxAxisKey previous_axis{};
    BoxAxisKey next_axis{};
    if (!decode_box_axis_feature_id_(previous_manifold.manifold_feature_id,
                                     previous_axis) ||
        !decode_box_axis_feature_id_(next_manifold.manifold_feature_id,
                                     next_axis)) {
      return false;
    }
    return previous_axis.type != next_axis.type ||
           previous_axis.i != next_axis.i || previous_axis.j != next_axis.j;
  }

  [[nodiscard]] static bool
  manifold_normal_changed_(const ContactManifold &previous_manifold,
                           const ContactManifold &next_manifold) noexcept {
    if (previous_manifold.point_count == 0u ||
        next_manifold.point_count == 0u) {
      return true;
    ...
    return dot(previous_normal, next_normal) <
           kPersistenceNormalSimilarityThreshold;
  }

  [[nodiscard]] static f32
  local_anchor_match_distance_sq_(const ContactPoint &lhs,
                                  const ContactPoint &rhs,
                                  const bool manifold_has_body_b) noexcept {
    const f32 anchor_delta_a_sq =
        (lhs.local_anchor_a - rhs.local_anchor_a).length_sq();
    if (!manifold_has_body_b) {
      return anchor_delta_a_sq;
    }
    const f32 anchor_delta_b_sq =
        (lhs.local_anchor_b - rhs.local_anchor_b).length_sq();
  ...
  static void copy_point_cache_(ContactPoint &dst,
                                const ContactPoint &src) noexcept {
  ...
  // manifold frame.
  [[nodiscard]] bool should_drop_persisted_point_(
      std::span<const Vec3> position, std::span<const Quat> orientation,
      const ContactManifold &manifold, const ContactPoint &next_point,
      const ContactPoint &previous_point) const noexcept {
    const u32 a = manifold.a;
    const Vec3 world_a_previous =
        position[a] + rotate(orientation[a], previous_point.local_anchor_a);
    const Vec3 world_a_next =
        position[a] + rotate(orientation[a], next_point.local_anchor_a);
    ...
    if (manifold.b != kInvalidBody) {
      const u32 b = manifold.b;
      const Vec3 world_b_previous =
          position[b] + rotate(orientation[b], previous_point.local_anchor_b);
      const Vec3 world_b_next =
          position[b] + rotate(orientation[b], next_point.local_anchor_b);
      ...
      const Vec3 tangential_previous =
          delta_previous - manifold.normal * normal_previous;
    ...
    const bool normal_break_exceeded =
        normal_separation > kPersistenceNormalBreakThreshold;
    const bool normal_drift_exceeded =
        normal_drift > kPersistenceNormalBreakThreshold;
    const bool tangential_drift_exceeded =
        tangential_delta.length_sq() >
        kPersistenceTangentialDriftBreakThresholdSq;
    return normal_break_exceeded || normal_drift_exceeded ||
           tangential_drift_exceeded;
  }

  void match_and_transfer_point_cache_(
      std::span<const Vec3> position, std::span<const Quat> orientation,
      ContactManifold &next_manifold,
      const ContactManifold &previous_manifold) const {
#ifndef NDEBUG
    if (next_manifold.point_count > kMaxManifoldPoints ||
        previous_manifold.point_count > kMaxManifoldPoints) {
      log::error(physics,
    ...
    const u8 all_previous_used_mask =
        static_cast<u8>((1u << previous_point_count) - 1u);
    ...
    // fallback.
    auto try_match_point = [&](const u32 next_index,
                               const bool match_feature_first) {
      ...
      for (u32 previous_index = 0; previous_index < previous_point_count;
           ++previous_index) {
        const u8 previous_bit = static_cast<u8>(1u << previous_index);
        if ((previous_used_mask & previous_bit) != 0u) {
          continue;
        }
        const ContactPoint &previous_point =
            previous_manifold.points[previous_index];
        if (match_feature_first &&
            previous_point.feature_id != next_feature_id) {
          continue;
        }

        const f32 metric = local_anchor_match_distance_sq_(
            next_point, previous_point, manifold_has_body_b);
      ...
      const ContactPoint &previous_point =
          previous_manifold.points[best_previous];
      if (should_drop_persisted_point_(position, orientation, next_manifold,
  ...
  // diagnostics.
  [[nodiscard]] PersistenceRefreshStats
  refresh_manifold_persistence_(std::span<const Vec3> position,
    ...
    for (u32 i = 1; i < next_count; ++i) {
      ...
      if (manifold_pair_equal_(curr, prev)) {
        log::error(physics,
                   "Duplicate next manifold pair (index={} pair=({}, {}))", i,
                   curr.a, curr.b);
        std::terminate();
      }
    }
    ...
    const bool disjoint_pair_ranges =
        manifold_pair_less_(previous_last, next_first) ||
        manifold_pair_less_(next_last, previous_first);
    ...
    u32 next_index = static_cast<u32>(
        std::lower_bound(next_manifolds_.begin(), next_manifolds_.end(),
                         previous_first, manifold_pair_less_) -
    ...
    u32 previous_index = static_cast<u32>(
        std::lower_bound(manifolds_.begin(), manifolds_.end(),
                         next_manifolds_[next_index], manifold_pair_less_) -
        manifolds_.begin());
    while (next_index < next_count && previous_index < previous_count) {
      ...
      if (!manifold_pair_equal_(previous_manifold, next_manifold)) {
        log::error(physics,
                   ...
                   "(next=({}, {}) previous=({}, {}))",
                   next_manifold.a, next_manifold.b, previous_manifold.a,
                   previous_manifold.b);
        std::terminate();
      }
#endif
      const bool axis_flipped =
          manifold_axis_flipped_(previous_manifold, next_manifold);
      if (axis_flipped) {
        ++stats.axis_flip_count;
      }
      if (axis_flipped ||
          manifold_normal_changed_(previous_manifold, next_manifold)) {
        reset_manifold_point_cache_(next_manifold);
      ...
      match_and_transfer_point_cache_(position, orientation, next_manifold,
                                      previous_manifold);
      for (u32 i = 0; i < next_manifold.point_count; ++i) {
        stats.matched_point_count +=
            next_manifold.points[i].persisted ? 1u : 0u;
      }
      ++previous_index;
      ++next_index;
    }
  ...
  void ensure_broad_phase_workers_(const u32 count) {
    if (broad_phase_worker_count_ == 0) {
      ...
      broad_phase_worker_count_ = (hw_threads > 0u) ? usable_threads : 1u;
      broad_phase_workers_.resize(broad_phase_worker_count_);
      broad_phase_threads_.reserve(broad_phase_worker_count_ - 1u);
      broad_phase_pair_offsets_.reserve(
          static_cast<usize>(broad_phase_worker_count_) + 1u);
      start_broad_phase_workers_();
    }
    if (count <= broad_phase_worker_reserve_count_) {
      reserve_broad_phase_worker_pair_buffers_(
          broad_phase_worker_pair_reserve_hint_);
      return;
    }
    for (auto &worker : broad_phase_workers_) {
    ...
    const usize pair_capacity_floor =
        static_cast<usize>(count) * kPairReserveFactor;
    update_reserve_hint_(pair_capacity_floor,
                         broad_phase_worker_pair_reserve_hint_);
    reserve_broad_phase_worker_pair_buffers_(
        broad_phase_worker_pair_reserve_hint_);
  }

  void run_broad_phase_queries_(std::span<const u32> query_dynamic_ids,
                                std::span<const u8> query_dynamic_mask) {
    ...
    if (broad_phase_worker_count_ == 0u || broad_phase_workers_.empty()) {
      log::error(physics,
                 "Broad phase workers not initialized before query dispatch");
      std::terminate();
    }
#endif

    const BroadPhaseDispatch dispatch = choose_broad_phase_dispatch_(
        query_dynamic_count, broad_phase_worker_count_);
    const u32 worker_count = dispatch.worker_count;
    const u32 chunk_size = dispatch.chunk_size;
    TracyPlot("physics_broad_phase_workers_used",
              static_cast<i64>(worker_count));
    ...
    if (worker_count > 1) {
      {
        ...
        broad_phase_jobs_remaining_.store(worker_count - 1u,
    ...
    if (worker_count > 1) {
      ...
      broad_phase_done_cv_.wait(lock, [&] {
        return broad_phase_jobs_remaining_.load(std::memory_order_acquire) ==
               0u;
      });
    }

    {
      ...
      if (worker_count == 1u) {
        ...
        for (u32 worker_index = 0; worker_index < worker_count;
             ++worker_index) {
      ...
      update_reserve_hint_(max_worker_pair_count,
                           broad_phase_worker_pair_reserve_hint_);
      reserve_broad_phase_worker_pair_buffers_(
          broad_phase_worker_pair_reserve_hint_);
    }
  ...
  void run_broad_phase_chunk_(const BroadPhaseJob &job,
                              const u32 worker_index) {
    ...
    const std::span<const u32> chunk{job.query_dynamic_ids.data() + begin,
                                     end - begin};
    BroadPhaseWorker &worker = broad_phase_workers_[worker_index];
    broad_phase_generate_pairs(chunk, dynamic_bvh_, static_bvh_, bounds_cache_,
                               job.query_dynamic_mask, worker.pairs,
                               worker.scratch);
  }
  ...
  void broad_phase_worker_loop_(const u32 worker_index) {
    ...
    for (;;) {
      ...
      {
        std::unique_lock lock(broad_phase_mutex_);
        broad_phase_cv_.wait(lock, [&] {
          return broad_phase_stop_ || broad_phase_job_id_ != last_job;
        });
      ...
      if (broad_phase_jobs_remaining_.fetch_sub(
  ...
  void start_broad_phase_workers_() {
    ...
    for (u32 worker_index = 1; worker_index < broad_phase_worker_count_;
         ++worker_index) {
      broad_phase_threads_.emplace_back(
          [this, worker_index] { broad_phase_worker_loop_(worker_index); });
    }
...
#if defined(JAVELIN_BROAD_PHASE_VALIDATE)
  void validate_broad_phase_pairs_(std::span<const u32> awake_dynamic_ids,
                                   std::span<const u8> awake_dynamic_mask,
                                   const bool used_full_awake_query,
                                   const u64 step_id) {
    ...
    if (kBroadPhaseValidationIntervalTicks == 0u ||
        (step_id % static_cast<u64>(kBroadPhaseValidationIntervalTicks)) !=
            0u) {
      TracyPlot("physics_broad_phase_validator_ran", static_cast<i64>(0));
      return;
    }
    ...
    broad_phase_generate_pairs(awake_dynamic_ids, dynamic_bvh_, static_bvh_,
                               bounds_cache_, awake_dynamic_mask,
    ...
    if (broad_phase_expected_pairs_.size() != candidate_pairs_.size()) {
      log::error(physics,
                 "Incremental broad phase validation failed (step={} "
                 "expected_pairs={} actual_pairs={})",
                 step_id, broad_phase_expected_pairs_.size(),
                 candidate_pairs_.size());
      std::terminate();
    }
    for (usize i = 0; i < candidate_pairs_.size(); ++i) {
      if (!body_pair_equal_(broad_phase_expected_pairs_[i],
                            candidate_pairs_[i])) {
