...
struct Platform final {
  void init(const PlatformConfig &config = {}) {
    ...
    glfwSetMouseButtonCallback(window_.native, [](GLFWwindow *w,
                                                  const int button,
    ...
    glfwSetCursorPosCallback(window_.native, [](GLFWwindow *w, const double x,
                                                const double y) {
      if (auto *self = static_cast<Platform *>(glfwGetWindowUserPointer(w))) {
        self->input_.add_cursor_pos(static_cast<f32>(x), static_cast<f32>(y));
      }
    });
    glfwSetScrollCallback(window_.native, [](GLFWwindow *w, const double,
                                             const double y) {
      if (auto *self = static_cast<Platform *>(glfwGetWindowUserPointer(w))) {
        self->input_.add_scroll(static_cast<f32>(y));
      }
    });
    glfwSetWindowFocusCallback(window_.native, [](GLFWwindow *w,
                                                  const int focused) {
