...
public:
  ...
  void render_frame(const f64 dt, InputState &input) noexcept {
    ...
    resize_if_needed_();
    if (ui_enabled_) {
      build_ui_frame_(static_cast<f32>(dt * 1000.0), input);
    } else {
      input.end_frame(false, false);
    }

    update_camera_from_input_(input.frame(), static_cast<f32>(dt));

    const PoseSnapshot poses = scene_->pose_snapshot();
    draw_frame_(poses, compute_pose_alpha_(poses), true, ui_enabled_);
  }

  void render_offline_frame(const f64 dt,
                            const f32 pose_alpha = 1.0f) noexcept {
    ...
    resize_if_needed_();
    update_camera_without_input_(static_cast<f32>(dt));
...
private:
  ...
  void build_ui_frame_(const f32 render_dt_ms, InputState &input) noexcept {
  ...
  void update_camera_from_input_(const InputFrame &frame_input,
                                 const f32 dt) noexcept {
    if (camera_mode_ == CameraMode::orbit && !frame_input.ui_wants_mouse &&
    ...
    if (camera_mode_ == CameraMode::orbit) {
      ...
      cursor_mode = fly_camera_.update(camera_, frame_input, dt);
    }
  ...
  void draw_frame_(const PoseSnapshot &poses, const f32 pose_alpha,
    ...
    const FrameCamera frame_camera{
        .view = view, .proj = proj, .view_proj = proj * view};
    ...
    const ContactDebugSnapshot contacts =
        (physics_ != nullptr) ? physics_->contact_debug_snapshot()
                              : ContactDebugSnapshot{};
    const AabbDebugSnapshot aabbs = (physics_ != nullptr)
                                        ? physics_->aabb_debug_snapshot()
                                        : AabbDebugSnapshot{};
  ...
  tab_to_string_(const UiTab tab) noexcept {
    switch (tab) {
  ...
  [[nodiscard]] static bool parse_f32_(const std::string_view text,
                                       f32 &out) noexcept {
    if (text.empty()) {
      return false;
    }
  ...
  void load_ui_state_() noexcept {
    ...
    while (std::getline(in, line)) {
      ...
      const std::string_view value{line.data() + eq + 1u,
                                   line.size() - eq - 1u};
    ...
    if (has_x && has_y && has_w && has_h && loaded_w > 64.0f &&
        loaded_h > 64.0f) {
  ...
  [[nodiscard]] ImGuiTabItemFlags
  tab_item_flags_(const UiTab tab, const bool restore_pending) const noexcept {
  ...
  void handle_keyboard_shortcuts_(const UiSnapshot &snapshot,
                                  UiCommands &commands) const {
  ...
  [[nodiscard]] UiSnapshot
  build_ui_snapshot_(const f32 render_dt_ms) const noexcept {
    UiSnapshot snapshot{
    ...
    if (scene_ != nullptr) {
      ...
      snapshot.scene_body_count =
          static_cast<u32>(scene_view.generation.size());
      snapshot.scene_shape_count = static_cast<u32>(scene_view.shapes.size());
      snapshot.scene_constraint_count =
          static_cast<u32>(scene_view.constraints.size());
    }
    ...
    snapshot.completed_simulation_steps =
        physics_->completed_simulation_steps();
  ...
  void draw_debug_section_(DebugToggles &debug) const {
    ...
    ImGui::SameLine();
    help_marker_(
        "Enable or disable every debug and display toggle in this tab.");
    ...
    ImGui::SameLine();
    help_marker_(
        "Overlay sleeping bodies gray and recently-woken bodies yellow.");
  ...
  void draw_simulation_section_(const UiSnapshot &snapshot,
                                UiCommands &commands) const {
    if (!snapshot.has_physics) {
    ...
    ImGui::SameLine();
    help_marker_(
        "Pause or resume continuous fixed-rate simulation. Shortcut: F5.");
    ...
    ImGui::SameLine();
    help_marker_(
        "Advance exactly one fixed simulation tick (1/60 s). Shortcut: F6.");
    ImGui::SameLine();
    ...
    ImGui::SameLine();
    help_marker_(
        "Queue ten fixed simulation ticks while paused. Shortcut: F7.");
    ImGui::EndDisabled();
  ...
  void draw_parameters_section_(const UiSnapshot &snapshot,
                                UiCommands &commands) const {
    if (!snapshot.has_physics) {
      return;
    }
    ...
    if (ImGui::DragFloat("Linear Damping (1/s)", &linear_damping, 0.01f, 0.0f,
                         5.0f)) {
    ...
    if (ImGui::DragFloat("Angular Damping (1/s)", &angular_damping, 0.01f, 0.0f,
                         5.0f)) {
    ...
    ImGui::SameLine();
    help_marker_(
        "Exponential damping coefficient applied to angular velocity.");
  }
  ...
  void draw_performance_section_(const UiSnapshot &snapshot) {
    ...
    if (snapshot.physics_dt_ms > 0.0f &&
        snapshot.completed_simulation_steps !=
            last_physics_dt_sample_step_count_) {
    ...
    const int physics_history_count =
        physics_dt_history_full_ ? static_cast<int>(kPhysicsDtHistory)
  ...
  void draw_scene_section_(const UiSnapshot &snapshot,
                           UiCommands &commands) const {
    ...
    ImGui::SameLine();
    help_marker_(
        "Restore authored initial transforms and velocities. Shortcut: F9.");
    ImGui::EndDisabled();
  }
  ...
  debug_toggles_equal_(const DebugToggles &a, const DebugToggles &b) noexcept {
    return a.draw_grid == b.draw_grid && a.draw_contacts == b.draw_contacts &&
           a.draw_aabbs == b.draw_aabbs &&
           a.draw_sleep_state == b.draw_sleep_state &&
           a.draw_constraints == b.draw_constraints &&
           a.draw_velocities == b.draw_velocities &&
           a.apply_color_transform == b.apply_color_transform;
  }

  static void set_all_debug_toggles_(DebugToggles &debug,
  ...
  static void plot_dt_history_(const char *label, const f32 *history,
    ...
    for (int i = 0; i < history_count; ++i) {
      const usize idx = (static_cast<usize>(offset) + static_cast<usize>(i)) %
                        history_capacity;
    ...
    ImGui::PlotLines(label, history, history_count, offset, nullptr, plot_min,
                     plot_max, ImVec2(0, 80));
  }
...
private:
  ...
  // display. SleepDebugPass uses GL_LEQUAL depth (surface overlay) and must run
  // before the x-ray passes (ContactDebugPass, AabbDebugPass,
  // ConstraintDebugPass) which disable depth testing.
  using Pipeline =
      RenderPipeline<GeometryPass, WorldGridPass, SleepDebugPass,
                     ContactDebugPass, AabbDebugPass, ConstraintDebugPass,
                     VelocityDebugPass, DisplayPass>;
  ...
  // lag.
  static constexpr u64 kMaxPoseInterpolationSpanNs =
      2u * (1'000'000'000ull / 60u);
  // Last completed simulation step id that contributed a physics-dt history
  // sample. This avoids duplicating a single physics tick across many render
  // frames.
  ...
  compute_pose_alpha_(const PoseSnapshot &poses) noexcept {
    if (poses.count == 0 || poses.prev_time_ns == 0 ||
        poses.curr_time_ns == 0 || poses.curr_time_ns <= poses.prev_time_ns) {
      return 1.0f;
    }

    const u64 raw_span = poses.curr_time_ns - poses.prev_time_ns;
    const u64 interpolation_span =
        std::min(raw_span, kMaxPoseInterpolationSpanNs);
    ...
    const u64 interpolation_start_time =
        poses.curr_time_ns - interpolation_span;
    const u64 now = now_ns_();
    const u64 render_time =
        (now > interpolation_span) ? (now - interpolation_span) : 0u;
